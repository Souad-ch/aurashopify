...
export default async function ProductPage({
  ...
  const product = await prisma.product.findFirst({
    where: { id: params.id, storeId: store.id, status: "active" },
  });
  if (!product) notFound();
    ...
    >
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
        ...
        <div className="mt-6 grid gap-10 md:grid-cols-2">
          ...
          <div>
            {product.category && (
              <span className="badge bg-gray-100 text-ink-soft">
                {product.category}
              </span>
            )}
            ...
            <div className="mt-3 flex items-center gap-3">
              ...
              {product.compareAt && product.compareAt > product.price && (
                <span className="text-lg text-ink-soft line-through">
                  {formatCurrency(product.compareAt, store.currency)}
                </span>
              )}
            </div>
            ...
            <p className="mt-2 text-sm">
              {product.stock > 0 ? (
                <span className="text-brand-600">✓ متوفر في المخزون</span>
              ) : (
                <span className="text-rose-600">نفد من المخزون</span>
              )}
            </p>
